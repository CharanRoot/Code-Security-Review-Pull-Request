...
pub async fn run() -> anyhow::Result<()> {
    ...
    let trigger_phrase = env::var("trigger_phrase").unwrap_or("opensecbot review".to_string());
...
async fn handler(
    ...
    let system = &format!("You are a princpal application security enginner. You will review a source code file and its patch related to the subject of \"{}\".", title);
    ...
    resp.push_str("Hello, I am a secuirty review bot on [flows.network](https://flows.network/). Here are my reviews of changed source code files in this PR.\n\n------\n\n");
    match pulls.list_files(pull_number).await {
        Ok(files) => {
            for f in files.items {
                ...
                let question = "The following is a patch. Please recommand updated secure version of the code.\n\n".to_string() + t_patch_as_text;
                match openai.chat_completion(&chat_id, &question, &co).await {
                    Ok(r) => {
                        resp.push_str(&r.choice);
                        resp.push_str("\n\n");
